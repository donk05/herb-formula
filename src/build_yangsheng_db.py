"""
养生书 RAG 本地知识库构建脚本
将 data/25养生-18本/ 下的 18 本养生书 TXT 文件向量化存储到独立的 ChromaDB

与古籍库分离，避免单个 chroma_db 文件过大：
  - 古籍：data/chroma_db/（collection: ancient_books）
  - 养生：data/chroma_db_yangsheng/（collection: yangsheng_books）

技术栈（100% 本地 CPU）：
  - 文本切块：RecursiveCharacterTextSplitter (chunk=300, overlap=50)
  - 向量模型：BAAI/bge-small-zh-v1.5
  - 向量数据库：ChromaDB 持久化到 data/chroma_db_yangsheng/

用法：python src/build_yangsheng_db.py
"""
...
def main():
    books_dir = os.path.join(_project_root, "data", "25养生-18本")
    persist_dir = os.path.join(_project_root, "data", "chroma_db_yangsheng")
    ...
    print(f"📖 找到 {len(txt_files)} 本养生书，开始处理...\n")
